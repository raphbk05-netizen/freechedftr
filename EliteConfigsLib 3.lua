...
do
    ...
    Library.Create = function(Self, Class, Properties)
        -- UIStroke can't be parented to TextButton on some platforms
        if Class == "UIStroke" and Properties and Properties.Parent then
            local parent = Properties.Parent
            if typeof(parent) == "Instance" and (parent:IsA("TextButton") or parent:IsA("ImageButton")) then
                local Data = { Class = Class, Properties = Properties, Instance = Instance.new("Frame") }
                Data.Instance.Visible = false
                Data.Instance.Size = UDim2.new(0,0,0,0)
                return setmetatable(Data, {
                    __index = function(t, k)
                        if k == "Instance" then return t.Instance end
    ...
    Library:Thread(function()
        while Library and Library.BackgroundEffects do
            ...
            -- Tween to animate snowflake falling
...
-- Function to toggle snow effect visibility
Library.SetBackgroundEffectsVisible = function(Self, Bool, Instant)
    ...
    local IsSnowing = Bool
    Effects.IsSnowing = IsSnowing

    if IsSnowing then
        Effects.Background.Visible = true
    else
        -- Cleanup snowflakes when snowing stops
        for _, Child in pairs(Effects.SnowHolder:GetChildren()) do
            if Child:IsA("ImageLabel") then
                Child:Destroy()
            end
        end
        Effects.Background.Visible = false
    end
end
